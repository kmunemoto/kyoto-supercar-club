  ...
  return (
    <SiteShell>
      <PageIntro
        kicker="PRIVACY"
        title="プライバシーポリシー"
        lead={`${BRAND.name}（以下「当サービス」）は、興味登録、先行相談、お問い合わせなどを通じてお預かりする個人情報を、以下の方針に基づいて取り扱います。`}
      />
      <Section className="prose-like max-w-3xl space-y-10 pt-0">
        <article>
          <h2 className="font-serif text-2xl">1. 取得する情報</h2>
          <p className="mt-3 text-ink-soft">
            共同所有の興味登録、REGISTRYの登録・先行相談、お問い合わせにおいて、氏名、メールアドレス、電話番号、居住地域（京都市／京都府内／京都府外）、京都府内の車両保管地域、車両の概要、関心事項、自己申告の運転歴、広告流入時のUTMパラメータ、参照元、自由記述を取得します。初回の段階では、正確な保管住所、ナンバープレート、車台番号、車検証、免許証画像、資産証明、保証金、決済情報は取得しません。
          </p>
        </article>
        <article>
