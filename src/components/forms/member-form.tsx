...
export function MemberForm() {
  ...
  async function onSubmit(e: FormEvent) {
    e.preventDefault();
    const parsed = memberPreregSchema.safeParse(form);
    if (!parsed.success) {
  ...
  return (
    <form onSubmit={onSubmit} className="relative space-y-8" noValidate>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Field label="年齢" htmlFor="age" required hint="現時点の想定は30歳以上です。" error={errors["age"]}>
        ...
        <Field label="運転免許取得年数" htmlFor="licenseYears" required hint="現時点の想定は5年以上です。" error={errors["licenseYears"]}>
          <Input id="licenseYears" type="number" value={form.licenseYears} onChange={(e) => set("licenseYears", e.target.value)} />
        </Field>
      </div>
      <Field label="希望する利用頻度" htmlFor="useFrequency" required error={errors["useFrequency"]}>
        <NativeSelect id="useFrequency" value={form.useFrequency} onChange={(e) => set("useFrequency", e.target.value)}>
          {USE_FREQUENCY.map((r) => (
            <option key={r}>{r}</option>
          ))}
        </NativeSelect>
      ...
      <fieldset>
        ...
        <p className="mb-3 text-sm text-muted">具体的なメーカー名の希望は、まだ確定ラインナップがないため伺いません。</p>
        ...
        {errors["interestModels"] ? <p className="mt-2 text-sm text-oxblood">{errors["interestModels"]}</p> : null}
      </fieldset>
      <Field
        label="希望する料金帯"
        htmlFor="budgetBand"
      ...
      >
        <NativeSelect id="budgetBand" value={form.budgetBand} onChange={(e) => set("budgetBand", e.target.value)}>
          {BUDGET_BANDS.map((r) => (
            <option key={r}>{r}</option>
          ))}
        </NativeSelect>
      ...
      <Field label="利用目的" htmlFor="usePurpose" required error={errors["usePurpose"]}>
        <NativeSelect id="usePurpose" value={form.usePurpose} onChange={(e) => set("usePurpose", e.target.value)}>
          {USE_PURPOSES.map((r) => (
            <option key={r}>{r}</option>
          ))}
        </NativeSelect>
      ...
      >
        <NativeSelect id="incidentHistory" value={form.incidentHistory} onChange={(e) => set("incidentHistory", e.target.value)}>
      ...
      <Field label="サービスへの要望" htmlFor="requests" error={errors["requests"]}>
        <Textarea id="requests" value={form.requests} onChange={(e) => set("requests", e.target.value)} />
      </Field>
      <CheckRow>
